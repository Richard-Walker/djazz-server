...
{
	HANDLE handle;
	DWORD desired_access = (GENERIC_WRITE | GENERIC_READ); // (enumerate)? 0: (GENERIC_WRITE | GENERIC_READ);
	DWORD share_mode =FILE_SHARE_READ|FILE_SHARE_WRITE; // (enumerate)?
...
struct hid_device_info HID_API_EXPORT * HID_API_CALL hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	for (;;) {
		...
		//swprintf( buf,	L"Product/Vendor: %x %x\n", attrib.ProductID, attrib.VendorID);
...
HID_API_EXPORT hid_device * HID_API_CALL hid_open_path(const char *path)
{
	...
	if (dev->device_handle == INVALID_HANDLE_VALUE) {
		...
		sprintf(buf, "CreateFile: %d", GetLastError());
	...
	if (!res) {
		OutputDebugString(L"HidD_GetPreparsedData");
	...
	if (nt_res != HIDP_STATUS_SUCCESS) {
		...
		OutputDebugString(L"HidP_GetCaps");
		goto err_pp_data;
	}
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
	...
	if (!dev->read_pending) {
		...
		if (!res) {
			if (GetLastError() != ERROR_IO_PENDING) {
	char buf[100];
		sprintf(buf, "hid_read_timeout: %d", GetLastError());
		OutputDebugStringA(buf);
	...
	{
	char buf[100];
		sprintf(buf, "GetOverlappedResult: %d", GetLastError());
		OutputDebugStringA(buf);
	}
